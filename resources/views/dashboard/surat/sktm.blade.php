<x-layout>
  ...
  <div class="p-4 sm:ml-64">
    ...
    <div id="alert" class="md:w-1/2 mx-auto flex items-center p-4 mb-2 text-red-800 rounded-lg bg-red-200 transition-all duration-700" role="alert">
    ...
    <div id="alert" class="md:w-1/2 mx-auto flex items-center p-4 mb-2 text-green-800 rounded-lg bg-green-200 dark:bg-gray-800 dark:text-green-400" role="alert">
      <svg class="flex-shrink-0 w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
        <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"/>
      </svg>
      <span class="sr-only">Info</span>
          ...
          <tbody>
            @foreach ($sktm as $item)
              ...
              <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                <td class="px-6 py-4">
                  <button data-modal-target="ktp-modal" data-modal-toggle="ktp-modal" class="block text-blue-600 font-medium rounded-lg text-sm text-center" type="button" data-ktp="{{ asset('storage/'.$item->ktp) }}">
                    Lihat
                  </button> 
                </td>
                <td class="px-6 py-4">
                  {{-- <form action="{{ route('sktm.updateStatus', $item['id']) }}" method="POST" style="display: inline;"> --}}
                  <form action="/sktm/{{$item->id}}/acceptStatus" method="POST" style="display: inline;">
                    @csrf
                    @method('PATCH')
                    <button type="submit" class="font-medium text-green-600 hover:underline">Terima</button>
                  </form>
                  {{-- <form action="/sktm/{{$item->id}}/rejectStatus" method="POST">
                    @csrf
                    @method('PATCH') --}}
                    <button data-modal-target="reject-modal" data-modal-toggle="reject-modal" class="font-medium text-red-600 hover:underline" data-id="{{ $item->id }}">Tolak</button>
                  {{-- </form> --}}
                </td>
              </tr>
              @endif
          ...
          <tbody>
            ...
            @foreach ($sktm as $item)
              ...
              <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                <td class="px-6 py-4">
                  <a href="/dashboard/sktm/{{ $item->id }}/cetak" class="font-medium text-blue-600 hover:underline">Cetak</a>
                  <form action="/dashboard/sktm/{{ $item->id }}" method="post">
                    @method('delete')
                    @csrf
                    <button type="submit" class="font-medium text-red-600 hover:underline" onclick="return confirm('apakah anda yakin untuk menghapus ?')">Hapus</button>
                  </form>
                </td>
                <td class="px-6 py-4">
                  <button data-modal-target="authentication-modal" data-modal-toggle="authentication-modal" class="font-medium text-blue-600 hover:underline" type="button" data-email="{{ $item->email }}" data-nama="{{ $item->nama }}">
                    Kirim
                  </button>
                </td>
              </tr>
  ...
  <!-- modal ktp -->
  <div id="ktp-modal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="ktp-modal">
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                    </svg>
                    <span class="sr-only">Close modal</span>
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="reject-modal">
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                    </svg>
                    <span class="sr-only">Close modal</span>
            ...
            <form class="p-4 md:p-5" action="" method="POST" id="rejectForm">
                ...
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
                        <label for="alasan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Deskripsi</label>
                        <textarea name="alasan" id="alasan" rows="6" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Tulis disini" required></textarea>                    
                    </div>
                </div>
                <button type="submit" class="text-white inline-flex items-center bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-slate-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                    Kirim
                </button>
            </form>
        </div>
