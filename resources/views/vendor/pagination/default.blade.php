...
@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation"
        class="flex items-center justify-between p-4 border-0 select-none border-secondary-200 dark:border-secondary-600 sm:px-6">
        ...
        <div class="flex-col hidden lg:flex-row sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
                <p class="text-sm leading-5 dark:text-gray-300 hidden">
                ...
                <span class="relative z-0 inline-flex mt-2 shadow-sm lg:mt-0">
                    ...
                    @else
                        ...
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
                            ...
                            <svg class="w-5 h-5 text-secondary-200 fill-secondary-200" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                                    d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z"
                                    clip-rule="evenodd" />
                            </svg>
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span
                                            class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-white bg-secondary-200 border border-gray-300 dark:text-gray-400 dark:border-secondary-600 dark:bg-secondary-700">{{ $page }}</span>
                                    </span>
                                @else
                                    {{-- Array Of Links Enable --}}
                                    <a href="{{ $url }}"
                                        class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-secondary-200 transition bg-white border border-gray-300 hover:text-gray-400 dark:hover:text-gray-300 focus:z-10 focus:outline-none focus:border-primary-300 focus:ring focus:ring-primary-300 focus:ring-opacity-30 dark:focus:ring-primary-500 dark:focus:ring-opacity-30 dark:text-secondary-200 dark:border-secondary-600 dark:bg-secondary-700"
                    ...
                    @if ($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}#berita" rel="next"
                            ...
                            <svg class="w-5 h-5 text-secondary-200" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
